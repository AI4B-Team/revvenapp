...
import { Mic, Send, ChevronDown, Sparkles, Video, Pencil, Mic2, Move, User, Users, RefreshCw, BarChart, BookOpen, Headphones, Presentation, Image, Layers, Camera, ArrowRightLeft, AudioLines, Music, FileText, CreditCard, ImageIcon, LayoutTemplate, TableCellsMerge, Mail, FolderOpen, Box, Paintbrush, UserCircle, Link, Ratio, Hash, Clock, Settings2, Languages, Volume2, Gauge, MessageSquare, Upload, Captions, Grid, Folder } from 'lucide-react';
...
const contentTypes: SubOption[] = [
  { id: 'social', label: 'Social', icon: Grid, color: 'text-blue-500' },
];
    ...
    <TooltipProvider>
      <div className="w-full max-w-3xl mx-auto space-y-4">
        ...
        <div className="relative bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="px-4 py-2">
            <input
              ...
              className="w-full border-none text-base text-foreground bg-transparent font-body focus:outline-none placeholder:text-muted-foreground pt-2"
            />
          </div>
          ...
          <div className="flex items-center justify-between px-4 pb-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {selectedOption && (
                <ControlChip label={selectedOption.label} icon={selectedOption.icon} iconColor={selectedOption.color} onRemove={handleRemoveOption} />
              )}
            ...
            <div className="flex items-center gap-2">
              ...
              {hasPromptText && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-primary bg-primary/10 hover:bg-primary/20 transition-colors">
                      <Sparkles size={14} />
                      AI
                      <ChevronDown size={12} />
                    </button>
        ...
        {showTypeDropdown && showSubTypeSelector && (
          <div className="bg-card border border-border rounded-xl shadow-lg p-3 -mt-2">
