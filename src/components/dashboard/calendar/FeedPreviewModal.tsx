...
import { X, Grid3x3, LayoutGrid, Smartphone, Monitor, ChevronLeft, ChevronRight } from 'lucide-react';
...
const FeedPreviewModal: React.FC<FeedPreviewModalProps> = ({ isOpen, onClose, posts }) => {
  ...
  const renderInstagramGrid = () => (
    <div className="grid grid-cols-3 gap-0.5 bg-black">
      {platformPosts.slice(0, 9).map((post, index) => (
        <div 
          key={post.id} 
      ...
      <DialogContent className="sm:max-w-4xl max-h-[90vh] flex flex-col p-0">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 p-6 overflow-y-auto bg-muted/20 flex items-start justify-center">
            <div className={`${
              deviceView === 'mobile' 
                ? 'w-[375px] h-[700px] rounded-[40px] border-[8px] border-black bg-white dark:bg-zinc-950 overflow-hidden shadow-2xl'
              ...
              {/* Feed Content */}
              <div className="overflow-y-auto" style={{ maxHeight: deviceView === 'mobile' ? '600px' : 'none' }}>
                {renderPlatformFeed()}
              </div>
            </div>
          </div>
