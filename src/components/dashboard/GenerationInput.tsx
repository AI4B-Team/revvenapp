...
const GenerationInput = ({ selectedType, onCharactersClick, onCharactersSelect, selectedCharacters = [], onReferencesClick, onReferencesSelect, selectedReferences = [], isCharacterReference, onGenerationStart, externalStartingFrame, onContentTypeChange, onSocialGenerate }: GenerationInputProps) => {
  ...
  // Podcast mode (same logic as Story mode)
  const [podcastScenes, setPodcastScenes] = useState<{ scene: string; duration: number }[]>([{ scene: '', duration: 10 }]);
  const [podcastDuration, setPodcastDuration] = useState<'10' | '15' | '25'>('10');
  const [podcastReferenceImage, setPodcastReferenceImage] = useState<{ url: string; name: string; id?: string } | null>(null);
  const [isUploadingPodcastReference, setIsUploadingPodcastReference] = useState(false);
  const [isPodcastReferencePopoverOpen, setIsPodcastReferencePopoverOpen] = useState(false);
...
Make it look like a natural, professional product showcase or UGC-style promotional image with the character prominently featuring the product.`;
        ...
        if (selectedAnimateMode === 'Podcast') {
          // Validate: scenes must have meaningful content (at least 10 characters)
          const validScenes = podcastScenes.filter(s => s.scene.trim().length >= 10);
          const shortScenes = podcastScenes.filter(s => s.scene.trim().length > 0 && s.scene.trim().length < 10);
          ...
          console.log("Podcast Mode: Starting video generation with sora-2-pro-storyboard...");
          ...
          // Get image URL: character OR reference image (mutually exclusive)
          const podcastImageUrl = currentCharacters.length > 0 
            ? (currentCharacters[0].avatar || currentCharacters[0].image_url || currentCharacters[0].image)
            : podcastReferenceImage?.url || videoModeState.startingFrame?.preview || null;
          ...
          // Call the generate-video edge function with Podcast-specific parameters (same as Story)
          const podcastRequestBody = {
            isStory: true, // Uses same backend logic as Story
            isPodcast: true, // Flag for identification
            userId: user.id,
            shots: shots,
            nFrames: podcastDuration,
            aspectRatio: videoAspectRatio === 'portrait' ? 'portrait' : 'landscape',
            imageUrls: podcastImageUrl ? [podcastImageUrl] : undefined
          };

          const { data: podcastData, error: podcastError } = await supabase.functions.invoke('generate-video', {
          ...
          toast({
            title: "Podcast video generating!",
            description: "Your podcast video is being created. This may take a few minutes.",
          });

          console.log("Podcast video generation started:", podcastData);
          
          // Clear Podcast state after successful generation
          setPodcastScenes([{ scene: '', duration: 7.5 }]);
              ...
              <>
                ...
                <TooltipProvider>
                  ...
                  {selectedAnimateMode === 'Avatar Video' ? (
                    ...
                    <>
                      {/* Podcast Mode Controls - uses sora-2-pro-storyboard (same as Story) */}
                      <button className="px-4 py-1.5 rounded-full text-sm transition flex items-center gap-2 whitespace-nowrap bg-pill-orange text-pill-orange-text hover:opacity-80">
                        <Mic size={14} />
                        Sora Storyboard
                      </button>
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>{podcastReferenceImage ? 'Clear reference to select character' : 'Select Character'}</p>
                        </TooltipContent>
                      </Tooltip>
                      ...
                      {videoModeState.characters.length > 0 ? (
                        ...
                        <button 
                          onClick={() => setPodcastReferenceImage(null)}
                          className="px-4 py-1.5 rounded-full text-sm transition flex items-center gap-2 whitespace-nowrap bg-pill-green text-pill-green-text hover:opacity-80"
                        >
                          <img 
                            src={podcastReferenceImage.url} 
                            alt={podcastReferenceImage.name} 
                            className="w-5 h-5 rounded object-cover"
                          />
                          <span className="max-w-20 truncate">{podcastReferenceImage.name}</span>
                          <X size={14} />
                        </button>
                      ) : (
                        <Popover open={isPodcastReferencePopoverOpen} onOpenChange={setIsPodcastReferencePopoverOpen}>
                          <PopoverTrigger asChild>
                            <button className="px-4 py-1.5 rounded-full text-sm transition flex items-center gap-2 whitespace-nowrap bg-pill-gray text-pill-gray-text hover:opacity-80">
                              <ImageIcon size={14} />
                              Reference
                          ...
                          <PopoverContent className="w-72 bg-background border-border z-50 max-h-80 overflow-y-auto">
                            <div className="space-y-3">
                              ...
                              <input
                                ...
                                accept="image/*"
                                className="hidden"
                                id="podcast-reference-upload"
                                onChange={async (e) => {
                                  const file = e.target.files?.[0];
                                  if (!file) return;
                                  
                                  setIsUploadingPodcastReference(true);
                                  try {
                                    const formData = new FormData();
                                    formData.append('file', file);
                                    formData.append('upload_preset', 'revven');
                                    ...
                                    setIsPodcastReferencePopoverOpen(false);
                                    ...
                                    setIsUploadingPodcastReference(false);
                                    e.target.value = '';
                                  }
                                }}
                              />
                              <label 
                                htmlFor="podcast-reference-upload"
                                className="flex items-center gap-2 p-3 border border-dashed border-border rounded-lg cursor-pointer hover:bg-secondary/50 transition"
                              >
                                {isUploadingPodcastReference ? (
                                  <Loader2 size={20} className="animate-spin text-muted-foreground" />
                                ) : (
                                  <Upload size={20} className="text-muted-foreground" />
                                )}
                                <span className="text-sm text-muted-foreground">
                                  {isUploadingPodcastReference ? 'Uploading...' : 'Click to upload new'}
                                </span>
                              </label>
                                    ...
                                    {isLoadingReferenceImages ? (
                                      ...
                                      savedReferenceImages.map((ref) => (
                                        <button
                                          key={ref.id}
                                          onClick={() => {
                                            console.log('Podcast mode: Selecting reference image:', ref);
                                            setPodcastReferenceImage({
                                              url: ref.url,
                                              name: ref.name,
                                              id: ref.id
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <button className={`px-4 py-1.5 rounded-full text-sm transition flex items-center gap-2 whitespace-nowrap ${
                            podcastScenes.some(s => s.scene.trim().length > 0)
                              ? 'bg-pill-green text-pill-green-text' 
                              : 'bg-pill-gray text-pill-gray-text'
                          } hover:opacity-80`}>
                            <Clapperboard size={14} />
                            Scenes ({podcastScenes.length})
                            <ChevronDown size={14} />
                          </button>
                        </PopoverTrigger>
                        <PopoverContent className="w-96 bg-background border-border z-50 max-h-[400px] overflow-y-auto">
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <p className="text-sm font-medium">Podcast Scenes</p>
                              <button
                                onClick={() => {
                                  const maxDuration = parseInt(podcastDuration);
                                  const newSceneCount = podcastScenes.length + 1;
                                  const splitDuration = parseFloat((maxDuration / newSceneCount).toFixed(1));
                                  // Redistribute duration evenly across all scenes
                                  const updatedScenes = podcastScenes.map(s => ({ ...s, duration: splitDuration }));
                                  updatedScenes.push({ scene: '', duration: splitDuration });
                                  setPodcastScenes(updatedScenes);
                                }}
                                className="flex items-center gap-1 text-xs text-primary hover:underline"
                              >
                                <Plus size={12} />
                            ...
                            <p className={`text-xs ${
                              Math.abs(podcastScenes.reduce((sum, s) => sum + s.duration, 0) - parseInt(podcastDuration)) > 0.5
                                ? 'text-destructive font-medium'
                                : 'text-muted-foreground'
                            }`}>
                              Total: {podcastScenes.reduce((sum, s) => sum + s.duration, 0).toFixed(1)}s / {podcastDuration}s
                              {Math.abs(podcastScenes.reduce((sum, s) => sum + s.duration, 0) - parseInt(podcastDuration)) > 0.5 && ' ⚠️ Must equal total'}
                            </p>
                            {podcastScenes.map((scene, index) => (
                              <div key={index} className="space-y-2 p-3 bg-muted/50 rounded-lg">
                                <div className="flex items-center justify-between">
                                  <span className="text-xs font-medium text-muted-foreground">Scene {index + 1}</span>
                                  {podcastScenes.length > 1 && (
                                    <button
                                      onClick={() => setPodcastScenes(podcastScenes.filter((_, i) => i !== index))}
                                      className="text-destructive hover:text-destructive/80"
                                    >
                                      <Trash2 size={14} />
                                    </button>
                                ...
                                <textarea
                                  value={scene.scene}
                                  onChange={(e) => {
                                    const updated = [...podcastScenes];
                                    updated[index].scene = e.target.value;
                                    setPodcastScenes(updated);
                                  }}
                                  placeholder="Describe this scene..."
                                  className="w-full h-16 text-sm p-2 rounded-md border border-border bg-background resize-none"
                                />
                                <div className="flex items-center gap-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      const newDuration = Math.max(1, parseFloat(e.target.value) || 1);
                                      const otherScenesDuration = podcastScenes.reduce((sum, s, i) => i === index ? sum : sum + s.duration, 0);
                                      const maxAllowed = parseInt(podcastDuration) - otherScenesDuration;
                                      const updated = [...podcastScenes];
                                      updated[index].duration = Math.min(newDuration, Math.max(1, maxAllowed));
                                      setPodcastScenes(updated);
                                    }}
                                    min={1}
                                    max={parseInt(podcastDuration) - podcastScenes.reduce((sum, s, i) => i === index ? sum : sum + s.duration, 0)}
                                    step={0.5}
                                    className="w-16 text-sm p-1 rounded-md border border-border bg-background"
                                  />
                                  <span className="text-xs text-muted-foreground">sec</span>
                      ...
                      <Popover>
                        ...
                        <PopoverContent className="w-36 bg-background border-border z-50">
                          <div className="space-y-1">
                            <button 
                              onClick={() => setPodcastDuration('10')}
                              className={`w-full px-3 py-2 text-sm text-left hover:bg-secondary rounded-md transition ${podcastDuration === '10' ? 'bg-secondary' : ''}`}
                            >
                              10 seconds
                            </button>
                            <button 
                              onClick={() => setPodcastDuration('15')}
                              className={`w-full px-3 py-2 text-sm text-left hover:bg-secondary rounded-md transition ${podcastDuration === '15' ? 'bg-secondary' : ''}`}
                            >
                              15 seconds
                            </button>
                            <button 
                              onClick={() => setPodcastDuration('25')}
                              className={`w-full px-3 py-2 text-sm text-left hover:bg-secondary rounded-md transition ${podcastDuration === '25' ? 'bg-secondary' : ''}`}
                            >
                              25 seconds
                            </button>
                          </div>
                      ...
                      (isVideoMode && selectedAnimateMode === 'Avatar Video' 
                        ...
                        : isVideoMode && selectedAnimateMode === 'UGC'
                          ...
                          : isVideoMode && selectedAnimateMode === 'Story'
                            ...
                            : isVideoMode && selectedAnimateMode === 'Podcast'
                              ? (!podcastScenes.some(s => s.scene.trim().length >= 10) || (videoModeState.characters.length === 0 && !podcastReferenceImage) || Math.abs(podcastScenes.reduce((sum, s) => sum + s.duration, 0) - parseInt(podcastDuration)) > 0.5)
                              : !prompt.trim()
                      )
                    }
