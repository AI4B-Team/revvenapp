...
import { 
  ...
  Type, List, QrCode, Video, Music, Table, Calendar, CheckSquare, GraduationCap, Award, BookOpen, Brain,
...
const ELEMENT_CATEGORIES: Record<string, { title: string; items: { id: string; name: string; icon: any; color: string }[] }> = {
  ...
  charts: {
    ...
    items: [
      ...
      { id: 'trending-chart', name: 'Trending', icon: TrendingUp, color: '#10B981' },
...
type SectionId = 'templates' | 'content' | 'image' | 'text' | 'video' | 'audio' | 'elements' | 'mockups' | 'translate' | 'learning';
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  onClick={() => {
                    setIsCollapsed(false);
                    setExpandedSections(new Set(['learning']));
                  }}
                  className="p-3 hover:bg-gray-100 transition-colors flex items-center justify-center"
                >
                    ...
                    >
                      ...
                      >
                        ...
                        {editingChapterId === chapter.id ? (
                          ...
                          <>
                            {/* Badge for typed chapters (Cover, TOC, Introduction, Summary) */}
                            {chapter.type ? (
                              <span className="px-2 py-0.5 text-sm font-medium rounded flex-shrink-0 whitespace-nowrap bg-gray-200 text-gray-700">
                            ...
                            {chapter.type && <span className="flex-1" />}
                          </>
                        )}
